  ...
  message: (placeholder, message) => {
    ...
    mainModule.db.get(`SELECT blacklist FROM servers WHERE id = ${message.guild.id}`, (err, row) => {
      ...
      if (row && row.blacklist) {
        ...
        const bWords = row.blacklist.split(' ');
        const infractions = [];
        const wordsFound = [];
        ...
        if (infractions.length > 0) {
          ...
          message.delete({reason: 'Word filter broken'})
              .then(msg => {
                ...
                utils.timedMute(member.user.id, member.guild.id, 600, true, mainModule.client.user, 'Word filter infraction');
                member.createDM()
                    .then(chan => {
                      chan.send({
                        embed: {
                          color: 1571692,
                          title: `Your message was deleted on ${msg.guild.name} for breaking the word filter`,
                    ...
                    .catch(e => console.log(`[ERROR] Failed to dm user ${e}`));
              })
