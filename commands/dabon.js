    ...
    if (hasMR || hasKICK) {
      if (hasArgs) {
        ...
        if (targetPos >= authorPos && msg.author.id !== msg.guild.ownerID) utils.sendResponse(msg, `${targetUsr.tag} has a higher or equal role. You also cannot dab on yourself`, 'err');
          ...
          const chan = await target.createDM()
                      .catch(err => utils.sendResponse(msg, `Failed to create DM with ${targetUsr.tag} with error ${err}, trying to kick anyway`, 'info'));
          ...
          }).catch(err => utils.sendResponse(msg, `Failed to DM ${targetUsr.tag} with error ${err}, trying to kick anyway`, 'info'));

          const member = await target.kick({ reason: reason })
                        .catch(err => utils.sendResponse(msg, `Failed to dab. Error: ${err}`, 'err'));
